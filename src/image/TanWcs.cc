...
geom::Point2D TanWcs::undistortPixel(geom::Point2D const & pix) const {
    ...
    std::size_t const fOrder = _sipA.rows();
    std::size_t const gOrder = _sipB.rows();
    ...
    for (std::size_t i = 0; i < fOrder; ++i) {
        for (std::size_t j = std::max(std::size_t(0), 2-i); j < fOrder - i; ++j) {
    ...
    for (std::size_t i = 0; i < gOrder; ++i) {
        for(std::size_t j = std::max(std::size_t(0), 2-i); j < gOrder - i; ++j) {
